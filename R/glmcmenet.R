glmcmenet <- function (xme, xcme, y, family=c("binomial", "poisson"),
                       ...
                       act.vec = rep(1,ncol(xme) + ncol(xcme)), beta0 = rep(0, ncol(xme) + ncol(xcme)), penalty.factor=rep(1,ncol(xme) + ncol(xcme)),
          it.max = 250, screen_ind=F,str=F)
{
  ...
  if (str) {
    ret <- cme_str(xme.sc, xcme.sc, y, family, lambda.sib, lambda.cou, gamma,
             tau, xme.sl, xcme.sl, beta0, act.vec, penalty.factor, max.lambda, it.max,
