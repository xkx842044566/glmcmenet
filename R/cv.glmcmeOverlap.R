                          ...
                          screen_ind=F,str=F)
{
  ...
  fitall <- glmcmenetOverlap(xme = xme, xcme = xcme, y,  family=family, lambda.sib = lambda.sib,
                      ...
                      it.max = it.max, screen_ind=screen_ind,str=F)
  obj$cme.fit <- fitall
  obj$select.idx <- which(fitall$beta[, which(lambda.sib ==
  ...
  class(obj) = c("cv.glmcmeOverlap","cv.glmcme")
  return(obj)
}
