...
double mcp(double beta, double v, double lambda, double gamma){
  double ret = 0.0;
  if (abs(beta) <= (lambda*gamma/v) ){
    ret = abs(beta) - pow(beta,2.0)/(2.0*lambda*gamma/v);
  }
  else{
    ret = lambda*gamma/(2.0*v);
  }
  return(ret);
}
    ...
    for (int k=0;k<(2*(pme-1));k++){ //conditioned effect
      ...
      if (act_cme[cmeind]){
        ...
        for (int l=0;l<nn;l++){
          ...
          resid[l] = (yy[l]-mu[l])/W[l];
          if (yy[l]==1) dev = dev - log(mu[l]);
        ...
        if (!dbleq(beta_cme[cmeind],cur_beta)){ // if beta changed...
          ...
          for (int ll=0;ll<nn;ll++){
            ...
            mu[ll] = pbinomial(eta[ll]) ;
            W[ll] = fmax2(mu[ll]*(1-mu[ll]),0.0001);
            //v += (X_me[j*nn+k]*W[k]*X_me[j*nn+k])/((double)nn);
          }
